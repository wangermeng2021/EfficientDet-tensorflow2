...
def get_y_true_with_multi_class(max_side, batch_boxes, groundtruth_valids,model_type, num_classes,anchor_match_type,anchor_match_iou_thr,anchor_match_wh_ratio_thr):
    ...
    if anchor_match_type == 'iou':
        iou_scores = box_utils.boxes_iou(batch_boxes, np.reshape(np.array(yolo_anchors[model_type]), [-1,2])/(max_side, max_side))
        matched_anchor_index = np.argsort(-iou_scores,axis=-1)
        matched_anchor_num = np.sum(iou_scores>anchor_match_iou_thr, axis=-1)
        if anchor_match_iou_thr == -1:
            matched_anchor_num = np.ones_like(matched_anchor_num)
    else:
        anchors_wh = np.reshape(np.array(yolo_anchors[model_type]), [-1, 2]) / (max_side, max_side)
        ...
        matched_anchor_num = np.sum(wh_ratio < anchor_match_wh_ratio_thr, axis=-1)
    ...
    if model_type=='tiny':
        grids_0 = np.zeros([batch_size, grid_size[0], grid_size[0], len(yolo_anchors[model_type][0]), 5 + class_num], np.float32)
        grids_1 = np.zeros([batch_size, grid_size[1], grid_size[1], len(yolo_anchors[model_type][0]), 5 + class_num], np.float32)
        ...
        grids_0 = np.zeros([batch_size, grid_size[0], grid_size[0], len(yolo_anchors[model_type][0]), 5 + class_num], np.float32)
        grids_1 = np.zeros([batch_size, grid_size[1], grid_size[1], len(yolo_anchors[model_type][0]), 5 + class_num], np.float32)
        grids_2 = np.zeros([batch_size, grid_size[2], grid_size[2], len(yolo_anchors[model_type][0]), 5 + class_num], np.float32)
        grids_3 = np.zeros([batch_size, grid_size[3], grid_size[3], len(yolo_anchors[model_type][0]), 5 + class_num], np.float32)
        grids_4 = np.zeros([batch_size, grid_size[4], grid_size[4], len(yolo_anchors[model_type][0]), 5 + class_num], np.float32)
    ...
    for batch_index in range(batch_size):
        for box_index in range(groundtruth_valids[batch_index]):
            for anchor_index in batch_boxes[batch_index][box_index][5:5 + int(batch_boxes[batch_index][box_index][-1])]:
                grid_index = (anchor_index // len(yolo_anchors[model_type][0])).astype(np.int32)
                grid_anchor_index = (anchor_index % len(yolo_anchors[model_type][0])).astype(np.int32)
...
def get_y_true_with_one_class(max_side, batch_boxes, groundtruth_valids,model_type, num_classes,anchor_match_type,anchor_match_iou_thr,anchor_match_wh_ratio_thr):
    if model_type == 'tiny':
        detect_layer_num = 2
        strides = [16 * 2 ** i for i in range(detect_layer_num)]
    if model_type == 'p5':
    ...
    if model_type == 'tiny':
        grids_0 = np.zeros(
            [batch_size, grid_size[0], grid_size[0], len(yolo_anchors[model_type][0]), 5 + class_num], np.float32)
        grids_1 = np.zeros(
            [batch_size, grid_size[1], grid_size[1], len(yolo_anchors[model_type][0]), 5 + class_num], np.float32)
        grids = [grids_0, grids_1]
    elif model_type == 'p5':
        grids_0 = np.zeros(
            [batch_size, grid_size[0], grid_size[0], len(yolo_anchors[model_type][0]), 5 + class_num], np.float32)
        grids_1 = np.zeros(
            [batch_size, grid_size[1], grid_size[1], len(yolo_anchors[model_type][0]), 5 + class_num], np.float32)
        grids_2 = np.zeros(
            [batch_size, grid_size[2], grid_size[2], len(yolo_anchors[model_type][0]), 5 + class_num], np.float32)
        grids = [grids_0, grids_1, grids_2]
    elif model_type == 'p6':
        grids_0 = np.zeros(
            [batch_size, grid_size[0], grid_size[0], len(yolo_anchors[model_type][0]), 5 + class_num], np.float32)
        ...
        grids_2 = np.zeros(
            [batch_size, grid_size[2], grid_size[2], len(yolo_anchors[model_type][0]), 5 + class_num], np.float32)
        grids_3 = np.zeros(
            [batch_size, grid_size[3], grid_size[3], len(yolo_anchors[model_type][0]), 5 + class_num], np.float32)
        grids = [grids_0, grids_1, grids_2, grids_3]
    else:
        grids_0 = np.zeros(
            [batch_size, grid_size[0], grid_size[0], len(yolo_anchors[model_type][0]), 5 + class_num], np.float32)
        ...
        grids_3 = np.zeros(
            [batch_size, grid_size[3], grid_size[3], len(yolo_anchors[model_type][0]), 5 + class_num], np.float32)
        grids_4 = np.zeros(
            [batch_size, grid_size[4], grid_size[4], len(yolo_anchors[model_type][0]), 5 + class_num], np.float32)
        grids = [grids_0, grids_1, grids_2, grids_3, grids_4]
